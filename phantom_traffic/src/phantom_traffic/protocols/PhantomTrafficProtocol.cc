...
#include "PhantomTrafficProtocol.h"
...
Define_Module(PhantomTrafficProtocol);


const int PhantomTrafficProtocol::BEACON_TYPE = 12345;

void PhantomTrafficProtocol::initialize(int stage)
{
    ...
    if (stage == 1) {
        ...
        if (plexe::Veins11pRadioDriver* driver = FindModule<plexe::Veins11pRadioDriver*>::findSubModule(getParentModule())) {
            driver->registerNode(myId);
        }
    }
}

PhantomTrafficProtocol::~PhantomTrafficProtocol()
{
...
void PhantomTrafficProtocol::handleSelfMsg(cMessage* msg)
{
...
void PhantomTrafficProtocol::sendPhantomTrafficMessage(int destinationAddress)
{
    sendDown(createBeacon(destinationAddress).release());
}

std::unique_ptr<BaseFrame1609_4> PhantomTrafficProtocol::createBeacon(int destinationAddress)
{
    ...
    PhantomTrafficBeacon* pkt = new PhantomTrafficBeacon();
...
void PhantomTrafficProtocol::handleUnicastMsg(BaseFrame1609_4* unicast)
{
    ...
    if (PhantomTrafficBeacon* epkt = dynamic_cast<PhantomTrafficBeacon*>(enc)) {
...
void PhantomTrafficProtocol::receiveSignal(cComponent* source, simsignal_t signalID, bool v, cObject* details)
{
...
void PhantomTrafficProtocol::handleMessage(cMessage* msg)
{
...
void PhantomTrafficProtocol::handleLowerMsg(cMessage* msg)
{
    handleUnicastMsg(check_and_cast<BaseFrame1609_4*>(msg));
}

void PhantomTrafficProtocol::handleUpperMsg(cMessage* msg)
{
    sendDown(check_and_cast<BaseFrame1609_4*>(msg));
}

void PhantomTrafficProtocol::messageReceived(PhantomTrafficBeacon* pkt, BaseFrame1609_4* unicast)
{
}

void PhantomTrafficProtocol::registerApplication(int applicationId, InputGate* appInputGate, OutputGate* appOutputGate, ControlInputGate* appControlInputGate, ControlOutputGate* appControlOutputGate)
{
